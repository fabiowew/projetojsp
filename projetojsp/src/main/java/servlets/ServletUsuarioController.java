	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (acao != null && !acao.isEmpty() && acao.equalsIgnoreCase("deletar")) {
				...
				// aqui sempre retorna o total de pagina
				...
				// Será enviado por fora da lista na resposta do cabeçalho! (.addheader)
				request.setAttribute("totalPagina", daoUsuarioRepository.totalPagina(this.getUserLogado(request)));
				...
				if (modelLogin.getFotouser() != null && !modelLogin.getFotouser().isEmpty()) {
					...
					response.setHeader("Content-Disposition",
							"attachamente;filename=arquivo." + modelLogin.getExtensaofotouser());
				...
				response.setHeader("Content-Disposition", "attachamente;filename=arquivo.pdf");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (request.getPart("fileFoto") != null) {
				...
				if (part.getSize() > 0) {
					byte[] foto = IOUtils.toByteArray(part.getInputStream());// Converte a Imagem par Byte
			...
			// modelLogin = daoUsuarioRepository.consultaUsuarioID(id,
			// super.getUserLogado(request));
			List<ModelLogin> modelLogins = daoUsuarioRepository.consultaUsuarioList(super.getUserLogado(request));
